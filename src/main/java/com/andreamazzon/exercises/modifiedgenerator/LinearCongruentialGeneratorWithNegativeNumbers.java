package com.andreamazzon.exercises.modifiedgenerator;

import com.andreamazzon.session3.encapsulation.lazyinitialization.LinearCongruentialGenerator;

/**
 * This class generates pseudo random integers by a linear congruential
 * generator. The difference with LinearCongruentialGenerator is indeed that
 * here we also accept negative numbers. Thus we make this class inherit from
 * LinearCongruentialGenerator, just overriding one method, subtracting modulus
 * from the numbers generated in the method of the parent class. Note that we
 * override the method getRandomNumberSequence() first calling the version of
 * the parent class, and then modifying its output. The call of the version of
 * the parent class is done by super.nameOfTheMethod. Also note that we don't
 * have not to modify any access modifier of the parent class. It inherits all
...
public class LinearCongruentialGeneratorWithNegativeNumbers extends LinearCongruentialGenerator {
	...
	/**
	 * getter method for the sequence of pseudo random integer numbers (they can
	 * also be negative).
	 *
